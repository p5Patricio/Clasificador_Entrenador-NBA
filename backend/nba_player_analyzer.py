...
radar_stats_categories = {
    ...
    "Tiro": ['FG_PCT', 'FG3_PCT', 'FT_PCT'],
    "Defensivas": ['STL', 'BLK', 'PF']
}
...
def create_radar_chart(player_name, stats_to_plot, cluster_avg_stats, projected_stats, stats_labels, player_cluster_role, title="Rendimiento del Jugador"):
    ...
    # Colores alineados con el radar del frontend: jugador (cian), clúster (morado), proyección (azul)
    color_player = '#22d3ee'   # cian
    color_cluster = '#a855f7'  # morado
    color_proj = '#3b82f6'     # azul

    ax.plot(angles, player_values, color=color_player, linewidth=2.5, label=f'{player_name} (Actual)')
    ax.fill(angles, player_values, color=color_player, alpha=0.20)

    ax.plot(angles, cluster_avg_values, color=color_cluster, linewidth=2, label=f'Promedio {player_cluster_role} ({cluster_avg_stats.name})')
    ax.fill(angles, cluster_avg_values, color=color_cluster, alpha=0.12)

    ax.plot(angles, projected_values, color=color_proj, linewidth=2, linestyle='--', label=f'{player_name} (Proyectado)')
    ax.fill(angles, projected_values, color=color_proj, alpha=0.10)
...
def analyze_player_weak_spots(player_name, player_data_with_clusters, cluster_means, cluster_roles, stats_columns, scaler, threshold_multiplier=0.75):
    ...
    player_stats_raw = player_row[stats_columns].iloc[0]
    
    cluster_avg_stats_raw = cluster_means.loc[player_cluster]
    cluster_avg_stats_raw.name = player_cluster
    ...
    for stat in stats_columns:
        if stat in player_stats_raw.index and stat in cluster_avg_stats_raw.index:
            ...
            if pd.isna(player_val) or pd.isna(cluster_avg_val):
                projected_stats_raw[stat] = player_val
                continue
            if cluster_avg_val == 0:
                if player_val == 0:
                    continue
                else:
                    if '_PCT' in stat and player_val == 0:
                         weak_areas.append(f"{stat}: {player_val:.3f} (Promedio Clúster: {cluster_avg_val:.3f}) - [Necesita mejorar {stat}]")
                         projected_stats_raw[stat] = 0.05
                    continue 
            
            if '_PCT' in stat:
                if player_data_with_clusters.loc[player_row.index[0], 'GP'] > 10 and (cluster_avg_val - player_val) > 0.05:
                    weak_areas.append(f"{stat}: {player_val:.3f} (Promedio Clúster: {cluster_avg_val:.3f}) - [Necesita mejorar puntería/eficiencia]")
                    projected_stats_raw[stat] = min(player_val + 0.03, cluster_avg_val + 0.01)
            elif stat in ['TOV', 'PF']:
                if player_val > (cluster_avg_val * 1.20) and cluster_avg_val > 0:
                    weak_areas.append(f"{stat}: {player_val} (Promedio Clúster: {cluster_avg_val:.2f}) - [Reducir {stat}]")
                    projected_stats_raw[stat] = max(player_val * 0.90, cluster_avg_val * 0.95)
            elif player_val < (cluster_avg_val * threshold_multiplier):
                weak_areas.append(f"{stat}: {player_val} (Promedio Clúster: {cluster_avg_val:.2f}) - [Necesita mejorar {stat}]")
                projected_stats_raw[stat] = min(player_val * 1.15, cluster_avg_val * 0.95)
    ...
    if weak_areas:
        ...
        for area_desc in weak_areas:
            ...
            if found_key:
                print(f"  - Área: {area_desc.split(':')[0]}")
                print(f"    Sugerencias:")
                detailed_drills_html += f"<li><strong>{area_desc.split(':')[0]}</strong><ul>"
    ...
    for category_name, stats_list in radar_stats_categories.items():
        ...
        max_for_category = max_for_category if max_for_category > 0 else 1
...
def generate_player_report_pdf(player_name, player_row, player_cluster, cluster_roles, player_stats_raw, 
    ...
    player_cluster_role = cluster_roles.get(player_cluster, "Rol Desconocido")
    """
    Genera un reporte PDF con el análisis de jugador.
    """
    img_filenames = []
    ...
    create_radar_chart_to_file(ax_global, player_name, normalized_player_stats_all_pdf, normalized_cluster_avg_stats_all_pdf, 
                               ...
                               player_cluster_role,
                               title=f'Rendimiento Global de {player_name} vs. Clúster y Proyección')
    fig_global.tight_layout(pad=3.0)
    ...
    for stat in [s for s in all_stats_columns if s in cluster_avg_stats_raw.index]:
        display_stat_name = STAT_NAMES_MAP.get(stat, stat)
    ...
    """.format(global_radar_img=global_radar_img, player_name=player_name)

    for category_name, stats_list in radar_categories.items():
        cat_radar_img = f"radar_{category_name.replace(' ', '_')}_{player_name.replace(' ', '_')}.png"
        if os.path.exists(cat_radar_img):
            report_html += f"""
            <div class="chart-container">
    ...
    try:
        with open(output_filename, "wb") as f:
            pisa_status = pisa.CreatePDF(
                    ...
                    dest=f,
                    encoding='utf-8',
                    link_callback=lambda uri, rel: os.path.join(os.getcwd(), uri)
            )
        if pisa_status.err:
